...
import { Download, Trash2, RotateCw, X, Send, Info, Hand, PenTool, Eraser, BookOpen, Calculator, Sliders } from 'lucide-react';
...
const COMPONENT_NAMES = {
    ...
    fuse: 'Запобіжник',
    inductor: 'Котушка дротяна',
    electromagnet: 'Електромагніт',
    motor: 'Електродвигун',
    bell: 'Електричний дзвінок',
    speaker: 'Гучномовець',
    socket: 'Штепсельне з\'єднання',
    terminals: 'Затискачі',
    switch: 'Вимикач, ключ',
    button: 'Кнопка',
...
const COMPONENT_PROPS = {
    ...
    galvanometer: { r: 0.1, v: 0 },
    wattmeter: { r: 0.1, v: 0 },
};
const INITIAL_CIRCUIT = {
    '15,10': { x: 15, y: 10, type: 'cell', rotation: 0 },
    '16,10': { x: 16, y: 10, type: 'wire', rotation: 0 },
    '17,10': { x: 17, y: 10, type: 'wire', rotation: 0 },
    '18,10': { x: 18, y: 10, type: 'wire', rotation: 0 },
    '18,11': { x: 18, y: 11, type: 'wire', rotation: 0 },
    '18,12': { x: 18, y: 12, type: 'wire', rotation: 0 },
    '17,12': { x: 17, y: 12, type: 'resistor', rotation: 0 },
    '16,12': { x: 16, y: 12, type: 'wire', rotation: 0 },
    '15,12': { x: 15, y: 12, type: 'resistor', rotation: 0 },
    '14,12': { x: 14, y: 12, type: 'wire', rotation: 0 },
    '14,11': { x: 14, y: 11, type: 'wire', rotation: 0 },
    '14,10': { x: 14, y: 10, type: 'wire', rotation: 0 },
};
export default function App() {
    const [cells, setCells] = useState(INITIAL_CIRCUIT);
    ...
    // Calculator State
    const [calcT, setCalcT] = useState(60);
    const [calcUOverride, setCalcUOverride] = useState('');
    ...
    const handleCellClick = useCallback((x, y) => {
        ...
        if (mode === 'edit') {
            if (cells[key] && ['resistor', 'rheostat', 'heater', 'lamp', 'fuse', 'bell', 'cell', 'battery', 'inductor', 'electromagnet', 'motor', 'speaker'].includes(cells[key].type)) {
                setEditingCell(key);
            }
            return;
        }
    ...
    const exportPDF = async () => {
        ...
        try {
            ...
            Object.entries(counts).forEach(([type, count]) => {
                ...
                li.textContent = `- ${COMPONENT_NAMES[type]}: ${count}`;
    ...
    const hasNeighbor = useCallback((x, y, dx, dy) => {
    ...
    const analyzeCircuit = useCallback(() => {
        ...
        const portKey = (x, y, p) => `${x},${y},${p}`;
        for (let y = 0; y < GRID_HEIGHT; y++) {
            for (let x = 0; x < GRID_WIDTH; x++) {
                if (x < GRID_WIDTH - 1)
                    union(portKey(x, y, 1), portKey(x + 1, y, 3));
                if (y < GRID_HEIGHT - 1)
                    union(portKey(x, y, 2), portKey(x, y + 1, 0));
            }
        }
        const components = [];
        Object.entries(cells).forEach(([key, cell]) => {
            ...
            if (type === 'wire') {
                const activePorts = [];
                if (hasNeighbor(x, y, 0, -1))
                    activePorts.push(0);
                if (hasNeighbor(x, y, 1, 0))
                    activePorts.push(1);
                if (hasNeighbor(x, y, 0, 1))
                    activePorts.push(2);
                if (hasNeighbor(x, y, -1, 0))
                ...
                if (type === 'switch' || type === 'terminals' || type === 'button') {
    ...
    const renderComponent = (cell, isToolIcon = false) => {
        ...
        if (type === 'wire') {
            ...
            return (<svg width="40" height="40" className="absolute inset-0 pointer-events-none overflow-visible">
        ...
        <g transform={`rotate(${rotation} 20 20)`} stroke="black" strokeWidth="2" fill="none">
          ...
          {type === 'cell' && (<g>
              <line x1="16" y1="10" x2="16" y2="30" strokeWidth="2"/>
              <line x1="24" y1="14" x2="24" y2="26" strokeWidth="4"/>
              <line x1="0" y1="20" x2="16" y2="20"/>
              <line x1="24" y1="20" x2="40" y2="20"/>
              <text x="10" y="12" fontSize="10" strokeWidth="0" fill="black">+</text>
              <text x="30" y="12" fontSize="10" strokeWidth="0" fill="black">-</text>
            </g>)}
          
          {type === 'battery' && (<g>
              <line x1="8" y1="10" x2="8" y2="30" strokeWidth="2"/>
              <line x1="14" y1="14" x2="14" y2="26" strokeWidth="4"/>
              <line x1="16" y1="20" x2="24" y2="20" strokeDasharray="2 2"/>
              <line x1="26" y1="10" x2="26" y2="30" strokeWidth="2"/>
              <line x1="32" y1="14" x2="32" y2="26" strokeWidth="4"/>
              <line x1="0" y1="20" x2="8" y2="20"/>
              <line x1="32" y1="20" x2="40" y2="20"/>
              <text x="4" y="12" fontSize="10" strokeWidth="0" fill="black">+</text>
              <text x="36" y="12" fontSize="10" strokeWidth="0" fill="black">-</text>
            </g>)}
          
          {type === 'resistor' && (<rect x="10" y="14" width="20" height="12" fill="white"/>)}
          ...
          {type === 'fuse' && (<g>
              <line x1="0" y1="20" x2="40" y2="20" strokeWidth="2"/>
              <rect x="10" y="14" width="20" height="12" fill="none"/>
            </g>)}

          {type === 'socket' && (<g>
              <line x1="0" y1="16" x2="18" y2="16"/>
              <polygon points="14,13 18,16 14,19" fill="none" stroke="black"/>
              <line x1="0" y1="24" x2="18" y2="24"/>
              <polygon points="14,21 18,24 14,27" fill="none" stroke="black"/>
              <line x1="22" y1="12" x2="22" y2="28"/>
              <line x1="26" y1="12" x2="26" y2="28"/>
              <line x1="26" y1="16" x2="40" y2="16"/>
          ...
          {type === 'bell' && (<g>
              <path d="M 10 14 A 10 10 0 0 1 30 14 Z" fill="white"/>
          ...
          {type === 'rheostat' && (<g>
              ...
              <polygon points="18,10 22,10 20,14" fill="black"/>
            </g>)}

          {type === 'inductor' && (<g>
              <path d="M 10 20 A 4 4 0 0 1 16 20 A 4 4 0 0 1 22 20 A 4 4 0 0 1 28 20 A 4 4 0 0 1 34 20" fill="none"/>
              <line x1="0" y1="20" x2="10" y2="20"/>
              <line x1="34" y1="20" x2="40" y2="20"/>
            </g>)}

          {type === 'electromagnet' && (<g>
          ...
          {type === 'galvanometer' && (<g>
              <circle cx="20" cy="20" r="10" fill="white"/>
              <line x1="20" y1="26" x2="20" y2="14"/>
              <polygon points="18,16 22,16 20,12" fill="black"/>
            </g>)}

          {type === 'wattmeter' && (<g>
              <circle cx="20" cy="20" r="10" fill="white"/>
              <text x="20" y="24" textAnchor="middle" fontSize="12" strokeWidth="0" fill="black" fontWeight="bold">W</text>
            </g>)}

          {type === 'motor' && (<g>
              <circle cx="20" cy="20" r="10" fill="white"/>
              <text x="20" y="23" textAnchor="middle" fontSize="10" strokeWidth="0" fill="black" fontWeight="bold">M</text>
              <line x1="15" y1="26" x2="25" y2="26"/>
            </g>)}

          {type === 'speaker' && (<g>
              <rect x="12" y="16" width="4" height="8" fill="white"/>
              <polygon points="16,16 24,10 24,30 16,24" fill="white"/>
    ...
    const renderGrid = () => {
        ...
        for (let y = 0; y < GRID_HEIGHT; y++) {
            for (let x = 0; x < GRID_WIDTH; x++) {
                ...
                grid.push(<div key={key} data-x={x} data-y={y} className={`border border-slate-200/50 flex items-center justify-center relative ${mode === 'pan' ? 'cursor-grab' : 'cursor-crosshair'} hover:bg-slate-100`} style={{ width: CELL_SIZE, height: CELL_SIZE, flexShrink: 0 }} onMouseDown={(e) => {
                        ...
                        if (cell && ['resistor', 'rheostat', 'heater', 'lamp', 'fuse', 'bell', 'cell', 'battery', 'inductor', 'electromagnet', 'motor', 'speaker'].includes(cell.type)) {
                            setEditingCell(key);
                        }
                    }} onContextMenu={(e) => {
                        e.preventDefault();
            ...
            {cell && !['wire', 'wire_node', 'wire_cross', 'switch', 'button', 'terminals', 'socket'].includes(cell.type) && (<div className={`absolute -top-2 left-1/2 -translate-x-1/2 bg-white/95 px-1.5 py-0.5 rounded text-[10px] font-bold whitespace-nowrap z-10 shadow-sm border ${cell.type === 'ammeter' || cell.type === 'galvanometer' ? 'text-emerald-600 border-emerald-200' :
                            cell.type === 'voltmeter' ? 'text-sky-600 border-sky-200' :
                            cell.type === 'wattmeter' ? 'text-purple-600 border-purple-200' :
                                'text-slate-700 border-slate-200 cursor-pointer hover:bg-indigo-50 hover:border-indigo-300 hover:text-indigo-700 transition-colors'}`} onClick={(e) => {
                            e.stopPropagation();
                            if (['resistor', 'rheostat', 'heater', 'lamp', 'fuse', 'bell', 'cell', 'battery', 'inductor', 'electromagnet', 'motor', 'speaker'].includes(cell.type)) {
                                setEditingCell(key);
                            }
                        }}>
                {cell.type === 'ammeter' && cellResult ? `${cellResult.i.toFixed(2)} А` : null}
                {cell.type === 'galvanometer' && cellResult ? `${(cellResult.i * 1000).toFixed(1)} мА` : null}
                {cell.type === 'voltmeter' && cellResult ? `${cellResult.v.toFixed(2)} В` : null}
                {cell.type === 'wattmeter' && cellResult ? `${(cellResult.v * cellResult.i).toFixed(2)} Вт` : null}
                {['resistor', 'rheostat', 'heater', 'lamp', 'fuse', 'bell', 'cell', 'battery', 'inductor', 'electromagnet', 'motor', 'speaker'].includes(cell.type) && (`${cell.value !== undefined ? cell.value : (COMPONENT_PROPS[cell.type].v > 0 ? COMPONENT_PROPS[cell.type].v : COMPONENT_PROPS[cell.type].r)} ${COMPONENT_PROPS[cell.type].v > 0 ? 'В' : 'Ом'}`)}
              </div>)}
          </div>);
            }
        }
    ...
    return (<div className="h-[100dvh] w-full bg-slate-50 flex flex-col font-sans text-slate-900 overflow-hidden">
      {/* Header */}
      <header className="bg-white border-b border-slate-200 px-4 py-3 flex items-center justify-between shadow-sm shrink-0 z-20">
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 bg-indigo-600 rounded-xl flex items-center justify-center text-white font-bold text-xl shadow-md select-none">
            ⚡
          </div>
          <div className="flex flex-col min-w-0">
            <h1 className="text-base sm:text-lg font-bold text-slate-800 leading-tight truncate">Закони струму</h1>
            <p className="text-[10px] sm:text-[11px] text-slate-500 truncate hidden sm:block">З'єднання, робота, потужність</p>
          </div>
        </div>
      ...
      <main className="flex-1 overflow-auto bg-slate-100 relative z-0">
        <div className="min-w-max min-h-max p-4 md:p-8 flex items-center justify-center">
          <div className="bg-white shadow-xl rounded-lg overflow-hidden border border-slate-200" style={{
            width: GRID_WIDTH * CELL_SIZE,
            height: GRID_HEIGHT * CELL_SIZE
        }}>
            <div ref={gridRef} className="relative w-full h-full flex flex-wrap" style={{
            width: GRID_WIDTH * CELL_SIZE,
            height: GRID_HEIGHT * CELL_SIZE,
            touchAction: mode === 'pan' ? 'auto' : 'none'
        }} onTouchMove={handleTouchMove} onDragOver={(e) => e.preventDefault()}>
              {renderGrid()}
            </div>
          </div>
        </div>
      ...
      {/* Bottom Toolbar */}
      <div className="bg-white border-t border-slate-200 shrink-0 pb-safe z-20 shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.05)] w-full overflow-hidden">
        {/* Controls Row */}
        <div className="flex items-center justify-between px-4 py-2 border-b border-slate-100 overflow-x-auto gap-4 w-full hide-scrollbar touch-pan-x" onWheel={(e) => {
          if (e.deltaY !== 0) {
            e.currentTarget.scrollLeft += e.deltaY;
          }
        }}>
          <div className="flex bg-slate-100 rounded-lg p-1 shrink-0">
            <button onClick={() => setMode('draw')} className={`flex items-center gap-1.5 px-3 py-1.5 rounded-md text-[10px] md:text-xs font-medium transition-colors ${mode === 'draw' ? 'bg-white shadow-sm text-indigo-600' : 'text-slate-500'}`}>
              <PenTool size={14}/> <span className="hidden sm:inline">Малювати</span>
            </button>
            <button onClick={() => setMode('erase')} className={`flex items-center gap-1.5 px-3 py-1.5 rounded-md text-[10px] md:text-xs font-medium transition-colors ${mode === 'erase' ? 'bg-white shadow-sm text-red-600' : 'text-slate-500'}`}>
              <Eraser size={14}/> <span className="hidden sm:inline">Стерти</span>
            </button>
            <button onClick={() => setMode('pan')} className={`flex items-center gap-1.5 px-3 py-1.5 rounded-md text-[10px] md:text-xs font-medium transition-colors ${mode === 'pan' ? 'bg-white shadow-sm text-indigo-600' : 'text-slate-500'}`}>
              <Hand size={14}/> <span className="hidden sm:inline">Рухати</span>
            </button>
            <button onClick={() => setMode('edit')} className={`flex items-center gap-1.5 px-3 py-1.5 rounded-md text-[10px] md:text-xs font-medium transition-colors ${mode === 'edit' ? 'bg-white shadow-sm text-indigo-600' : 'text-slate-500'}`}>
              <Sliders size={14}/> <span className="hidden sm:inline">Налаштувати</span>
            </button>
            <div className="w-px bg-slate-200 mx-1 my-1"></div>
            <button onClick={() => setCurrentRotation(r => (r + 90) % 360)} className="flex items-center gap-1.5 px-3 py-1.5 rounded-md text-[10px] md:text-xs font-medium text-slate-500 hover:text-indigo-600 transition-colors shrink-0">
          ...
          <button onClick={clearGrid} className="flex items-center gap-1.5 px-3 py-1.5 bg-red-50 text-red-600 hover:bg-red-100 rounded-lg text-[10px] md:text-xs font-medium transition-colors shrink-0">
            <Trash2 size={14}/>
            Очистити все
          </button>
        </div>
        ...
        <div className="flex overflow-x-auto w-full px-3 py-3 gap-3 hide-scrollbar items-center touch-pan-x" onWheel={(e) => {
          if (e.deltaY !== 0) {
            e.currentTarget.scrollLeft += e.deltaY;
          }
        }}>
          {Object.keys(COMPONENT_NAMES).map((type) => (<button key={type} onClick={() => {
                setSelectedTool(type);
                setMode('draw');
            }} className={`shrink-0 w-[64px] h-[64px] md:w-[72px] md:h-[72px] flex flex-col items-center justify-center rounded-xl border-2 transition-all ${selectedTool === type && mode === 'draw'
      ...
      {/* Edit Modal */}
      {editingCell && cells[editingCell] && (<div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4 animate-in fade-in duration-200">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-xs overflow-hidden flex flex-col">
            ...
            <div className="p-4">
              ...
              <input type="number" autoFocus defaultValue={cells[editingCell].value !== undefined ? cells[editingCell].value : (COMPONENT_PROPS[cells[editingCell].type].v > 0 ? COMPONENT_PROPS[cells[editingCell].type].v : COMPONENT_PROPS[cells[editingCell].type].r)} onChange={(e) => {
      ...
      {/* Info Modal */}
      {showInfo && (<div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4 animate-in fade-in duration-200">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-sm overflow-hidden flex flex-col">
            ...
            <div className="p-4 overflow-y-auto max-h-[60vh]">
              ...
              {Object.entries(componentCounts).length === 0 ? (<p className="text-xs text-slate-400 italic">Схема порожня</p>) : (<ul className="text-sm space-y-2">
                  {Object.entries(componentCounts).map(([type, count]) => (<li key={type} className="flex justify-between items-center bg-slate-50 px-3 py-2 rounded-lg border border-slate-100">
                      <span className="text-slate-700">{COMPONENT_NAMES[type]}</span>
      ...
      {/* Formulas Modal */}
      {showFormulas && (<div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4 animate-in fade-in duration-200">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-sm overflow-hidden flex flex-col">
            <div className="px-4 py-3 border-b border-slate-100 flex justify-between items-center bg-slate-50">
      ...
      {/* Calculator Modal */}
      {showCalculator && (<div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4 animate-in fade-in duration-200">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-md overflow-hidden flex flex-col max-h-[90vh]">
            ...
            <div className="p-4 overflow-y-auto flex-1 space-y-5">
              ...
              {circuitData.error ? (<div className="bg-rose-50 border border-rose-200 text-rose-700 p-4 rounded-xl text-sm font-medium flex items-center justify-center text-center">
                  {circuitData.error}
                </div>) : (<>
                  {/* Main Parameters */}
                  <div className="grid grid-cols-1 gap-3">
                    <div>
                      ...
                      <input type="number" value={calcT} onChange={e => setCalcT(Number(e.target.value))} className="w-full bg-slate-50 border border-slate-200 rounded-lg px-3 py-2 text-slate-800 focus:outline-none focus:border-amber-500 focus:ring-1 focus:ring-amber-500"/>
                    </div>
                  </div>
      ...
      {showClearConfirm && (
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4 animate-in fade-in duration-200">
          <div className="bg-white rounded-xl shadow-xl max-w-sm w-full p-6">
            <h3 className="text-lg font-bold text-slate-800 mb-2">Очистити схему?</h3>
            <p className="text-slate-600 mb-6 text-sm">Ви впевнені, що хочете видалити всі елементи? Цю дію неможливо скасувати.</p>
            <div className="flex justify-end gap-3">
              <button onClick={() => setShowClearConfirm(false)} className="px-4 py-2 text-sm font-medium text-slate-600 hover:bg-slate-100 rounded-lg transition-colors">
                Скасувати
              </button>
              <button onClick={() => { setCells({}); setShowClearConfirm(false); }} className="px-4 py-2 text-sm font-medium text-white bg-red-600 hover:bg-red-700 rounded-lg transition-colors">
                Очистити
              </button>
            </div>
          </div>
